...
このリポジトリでは AI（Claude）と AI（codex）でセカンドオピニオンを取りながら開発する方針。**確信が持てない設計判断や非自明なコード変更があったら、独走せず codex の意見も聞く**。

#### いつ codex に相談するか
- **設計段階**: アーキテクチャ判断 / ライブラリ選定 / 外部 API のフロー設計 / 失敗時のエラーパス → 「`codex exec`」で相談（自由記述）
- **コミット前**: 非自明な変更 / 設計を含む変更 → 「`codex review --uncommitted`」で落とし穴チェック
- **特にやるべきとき**:
  - 共通ライブラリ・パターンの導入
  - ファイル/ディレクトリ構造の変更
  - セキュリティ関連（secrets、外部API、認証フロー）
  - 失敗時の挙動・エラーパス・冪等性
  - HTML/フォーム解析など壊れやすい外部依存
  - 新規プロジェクト・新規モジュール追加時
...
#### 迷ったら投げるコストは安い
1分で投げて1分で返ってくる。バグや設計負債が後で発覚するより圧倒的に安い。**「迷ったら投げる」をデフォルトに**。
