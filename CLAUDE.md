...
session 内で完結しない / 翌日以降に持ち越す task は **必ず GitHub issue に登録する**。口頭・memory だけだと session 跨ぎで存在を忘れる。memory は補助として書いて OK だが、**user-facing reminder としては必ず issue を主にする**。
...
| `SLACK_CHANNEL_<PROJECT>` | プロジェクト別 channel (例: `SLACK_CHANNEL_STOCK`) |
...
2. `<project>/pyproject.toml` 作成 (`tech_catchup/pyproject.toml` をテンプレに)
