...
| [`point_sites/`](./point_sites/) | 日本のポイ活サイト + 抽選サイト自動化（adapter 構造）。本番稼働中 8 ポイ活 + 3 抽選 (chanceit / fruitmail_lottery / dreammail): moppy / hapitas / amefuri / pointtown / getmoney / fruitmail / warau / sugutama / chanceit / fruitmail_lottery / dreammail。pointincome は JP geofence で extract-only。Cookie rotation 永続化 + 加算検証 (3 層 degradation alert) + Playwright wizard (各 site の daily 系 + 抽選応募) | サイト別 (8:00〜10:30 JST) | Bot Token + `SLACK_CHANNEL_<SITE>` |
